...
while ($line = <INFILE>) #scan through PsiBlast-output line by line
{
	...
	$line=<INFILE>;
        ...
        while ($line=~/^\s+.+\s+CS$/) # Cycle in this loop until no new "Query:" lines are found
        {	    
	    $line=<INFILE>;
            if ($line!~/\s*$query_name\s+(\d+)\s+(\S+)\s+(\d*)/) 
	    {
            ...
            if ($3 eq "") 
	    {
                <INFILE>; <INFILE>; <INFILE>; <INFILE>; <INFILE>; $line=<INFILE>;
	    ...
	    #print "$2\n";
            <INFILE>; 
	    $line=<INFILE>;  next if($line=~/.+\s+CS$/); last  if ($line=~/^>>/);
	    $line=<INFILE>;  next if($line=~/.+\s+CS$/); last  if ($line=~/^>>/);
	    #$line=<INFILE>;  next if($line=~/.+\s+CS$/); last  if ($line=~/^>>/);
...
#       $new_hit =~ tr/Uu/Cc/;   # nicht mehr noetig in blast. Kann aber alignhits.pl zum abschmieren bringen.
	...
	#printf("%s\n",$new_hit);
        ...
        # Prepare name line of hit
	$nameline=~/^\s*(\S*)\s*(.*)/;                   # delete everything after first block
